...
- Desktop builds now route app-facing calls through Rust first. SQLite-only, media-byte, and common Lofty tag-read/write paths stay in Rust; Python is called only for expert work such as MusicBrainz/AcoustID matching, feed/download flows, embedded artwork/lyrics writes, specialized volume/artwork operations, and CLAP/Torch inference. Most Python actions are one-shot; CLAP analysis uses a Rust-managed persistent worker for the batch.
...
- Keep Python as the expert worker for CLAP/Torch inference, tricky embedded artwork/lyrics writes, specialized ReplayGain/artwork operations, and any library where the Python ecosystem is clearly safer than current Rust crates.
...
- Confirmed album completion display is Rust-owned for local inferred/stored counts while MusicBrainz online lookup remains Python-owned until golden fixtures exist.
