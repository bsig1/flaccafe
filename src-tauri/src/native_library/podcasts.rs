...
fn episode_with_subscription(
    ...
    episode_id: i64,
) -> Result<NativePodcastEpisode, String> {
    connection
        .query_row(
            r#"
...
#[tauri::command]
pub fn native_refresh_podcast_subscription(
    _state: State<'_, NativeLibraryState>,
    subscription_id: i64,
) -> Result<NativePodcastRefreshResponse, String> {
    let connection = open_database()?;
    let subscription = subscription_by_id(&connection, subscription_id)?;
    ...
    Ok(NativePodcastRefreshResponse {
...
#[tauri::command]
pub fn native_download_podcast_episode(
    _state: State<'_, NativeLibraryState>,
    episode_id: i64,
    download_folder: Option<String>,
) -> Result<NativePodcastEpisode, String> {
    ...
    let base_folder = download_folder
        .filter(|value| !value.trim().is_empty())
        .map(PathBuf::from)
        .unwrap_or_else(|| subscription_download_folder(&connection, episode.subscription_id).unwrap_or_else(|_| export_dir().join("podcasts")));
...
#[tauri::command]
pub fn native_delete_podcast_episode_download(
    _state: State<'_, NativeLibraryState>,
    episode_id: i64,
) -> Result<NativePodcastDeleteDownloadResponse, String> {
    let connection = open_database()?;
    let episode = episode_with_subscription(&connection, episode_id)?;
    let mut deleted_file = false;
    let mut missing_file = false;
    if let Some(local_path) = episode.local_path.as_deref().filter(|value| !value.is_empty()) {
        let path = PathBuf::from(local_path);
        if path.exists() {
            if !path.is_file() {
    ...
    Ok(NativePodcastDeleteDownloadResponse {
...
#[tauri::command]
pub fn native_ensure_podcast_episode_track(
    _state: State<'_, NativeLibraryState>,
    episode_id: i64,
...
fn ensure_episode_track_for_connection(
    ...
    if let Some(track_id) = episode.track_id {
        if connection
            .query_row("SELECT id FROM tracks WHERE id = ?", params![track_id], |row| {
                row.get::<_, i64>(0)
            })
            .ok()
            .is_some()
    ...
    let path_text = episode
        ...
        .map(str::trim)
        .filter(|value| !value.is_empty())
        .ok_or_else(|| "Download the episode before adding it to the local collection".to_string())?;
    ...
    if metadata_map
        ...
        .is_none()
    {
        metadata_map.insert("duration_seconds".to_string(), json!(episode.duration_seconds));
    }
...
fn parse_feed(payload: &str, feed_url: &str) -> Result<ParsedFeed, String> {
    let document =
        roxmltree::Document::parse(payload).map_err(|error| format!("Invalid feed XML: {error}"))?;
    ...
    for (index, node) in channel
        ...
        .enumerate()
    {
        let title = child_text(node, &["title"]).unwrap_or_else(|| format!("Episode {}", index + 1));
...
fn podcast_metadata_from_file(path: &Path, episode: &NativePodcastEpisode) -> Map<String, JsonValue> {
    ...
    map.insert("duration_seconds".to_string(), json!(episode.duration_seconds));
