...
fn device_sync_tracks(
    ...
    track_ids: Option<Vec<i64>>,
    limit: usize,
) -> Result<(Vec<NativeTrack>, BTreeMap<i64, Vec<NativeTrack>>), String> {
    let mut by_id = BTreeMap::<i64, NativeTrack>::new();
...
#[allow(clippy::too_many_arguments)]
pub fn native_sync_device_folder(
    ...
    apply: Option<bool>,
    limit: Option<usize>,
) -> Result<NativeDeviceSyncResponse, String> {
    ...
    for track in &tracks {
        ...
        let mut change = NativeDeviceSyncChange {
            track_id: track.id,
    ...
    if export_playlists {
        for playlist_id in playlist_ids {
            ...
            let mut export = NativeDeviceSyncPlaylistExport {
                playlist_id,
                name,
...
pub fn native_device_sync_devices() -> NativeDeviceSyncDevicesResponse {
    ...
    #[cfg(windows)]
    {
        for letter in b'A'..=b'Z' {
            ...
            if path.exists() {
                devices.push(NativeDeviceSyncDetectedDevice {
    ...
    NativeDeviceSyncDevicesResponse {
        devices,
        mtp_supported: false,
