...
#[derive(Serialize)]
pub(crate) struct NativeDiagnosticItem {
    key: String,
    label: String,
    ok: bool,
    message: String,
...
#[derive(Serialize)]
pub(crate) struct NativeStartupDiagnosticsResponse {
    ok: bool,
    generated_at: String,
    items: Vec<NativeDiagnosticItem>,
    log_path: String,
    app_data_path: String,
}

#[derive(Serialize)]
pub(crate) struct NativeLogTailResponse {
    path: String,
    exists: bool,
    lines: Vec<String>,
}
...
#[derive(Serialize)]
pub(crate) struct NativeSupportBundleResponse {
    bundle_path: String,
    file_count: usize,
}

#[derive(Serialize)]
pub(crate) struct NativeBackupResponse {
    backup_path: String,
}

#[derive(Serialize)]
pub(crate) struct NativeLocalDataResetResponse {
    reset: bool,
...
pub(crate) fn native_startup_diagnostics() -> Result<NativeStartupDiagnosticsResponse, String> {
    ...
    Ok(NativeStartupDiagnosticsResponse {
...
pub(crate) fn native_backend_log_tail(limit: usize) -> NativeLogTailResponse {
    let limit = limit.clamp(1, 2_000);
    let path = backend_log_path();
    if !path.exists() {
        return NativeLogTailResponse {
...
pub(crate) fn native_backup_database() -> Result<NativeBackupResponse, String> {
    ...
    Ok(NativeBackupResponse {
        backup_path: target.to_string_lossy().to_string(),
    })
}

pub(crate) fn native_reset_local_data(
    confirmation: Option<String>,
) -> Result<NativeLocalDataResetResponse, String> {
    if confirmation
        .as_deref()
        .unwrap_or_default()
        .trim()
    ...
    let _ = open_database()?;
    Ok(NativeLocalDataResetResponse {
        reset: true,
        backup_path,
...
pub(crate) fn native_create_support_bundle() -> Result<NativeSupportBundleResponse, String> {
    let export_dir = export_dir();
    fs::create_dir_all(&export_dir).map_err(|error| {
        format!(
            "Could not create export folder {}: {error}",
    ...
    add_json(
        &mut archive,
        "diagnostics.json",
        &serde_json::to_value(native_startup_diagnostics()?)
    ...
    Ok(NativeSupportBundleResponse {
...
fn diagnostic_item<F>(key: &str, label: &str, path: Option<&Path>, check: F) -> NativeDiagnosticItem
where
    F: FnOnce() -> Result<String, String>,
{
    match check() {
        Ok(message) => NativeDiagnosticItem {
            key: key.to_string(),
        ...
        Err(message) => NativeDiagnosticItem {
            key: key.to_string(),
            label: label.to_string(),
            ok: false,
            message,
