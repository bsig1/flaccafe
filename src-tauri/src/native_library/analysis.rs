...
struct AnalysisJob {
    ...
    skipped: i64,
    errors: Vec<String>,
    failed_tracks: Vec<NativeAudioAnalysisError>,
...
pub(crate) fn native_start_clap_analysis(
    body: JsonValue,
) -> Result<NativeAudioAnalysisProgress, String> {
    let request = AnalysisRequest::from_body(&body);
    let status = crate::python_worker::call_python_action_json(
        "get_clap_status",
        json!({ "deep": true }),
        None,
    )?;
...
pub(crate) fn native_get_clap_analysis(
    job_id: String,
) -> Result<NativeAudioAnalysisProgress, String> {
...
pub(crate) fn native_pause_clap_analysis(
    job_id: String,
) -> Result<NativeAudioAnalysisProgress, String> {
...
pub(crate) fn native_resume_clap_analysis(
    job_id: String,
) -> Result<NativeAudioAnalysisProgress, String> {
    update_analysis_job(&job_id, |job| {
        if !terminal(&job.status) {
...
pub(crate) fn native_cancel_clap_analysis(
    job_id: String,
) -> Result<NativeAudioAnalysisProgress, String> {
    update_analysis_job(&job_id, |job| {
        if !terminal(&job.status) {
        ...
        match outcome {
            ...
            Err(error) => {
                ...
                update_analysis_job(job_id, |job| {
                    ...
                    job.failed_tracks.push(NativeAudioAnalysisError {
...
impl AnalysisJob {
    fn snapshot(&self) -> NativeAudioAnalysisProgress {
