...
use super::types::*;
use super::{album_artwork, metadata, open_database, scan, NativeLibraryState};
...
#[tauri::command]
pub fn native_auto_tag_musicbrainz(
    state: State<'_, NativeLibraryState>,
    body: JsonValue,
) -> Result<NativeAutoTagResponse, String> {
    ...
    for track in tracks {
        let match_result = if fingerprint_only || track.acoustic_fingerprint.is_some() {
            match acoustid_key.as_deref() {
                Some(key) => lookup_acoustid_track(&track, key)
                    .or_else(|error| {
                        if fingerprint_only {
                            Err(error)
                        } else {
                ...
                None if fingerprint_only => Err(
                    "AcoustID API key is required for fingerprint-only matching".to_string(),
                ),
        ...
        match match_result {
            Ok(Some(proposal)) => {
                ...
                previews.push(NativeAutoTagPreview {
                    track_id: track.id,
            ...
            Ok(None) => previews.push(NativeAutoTagPreview {
                track_id: track.id,
                path: track.path.clone(),
...
#[tauri::command]
pub fn native_lookup_album_completion(
    _state: State<'_, NativeLibraryState>,
    album_id: i64,
) -> Result<NativeAlbumCompletionLookupResponse, String> {
    let connection = open_database()?;
...
pub fn native_album_artwork_candidates(album_id: i64) -> Result<JsonValue, String> {
    let connection = open_database()?;
    Ok(json!({
        "album_id": album_id,
...
pub fn native_search_album_artwork(album_id: i64) -> Result<JsonValue, String> {
    let connection = open_database()?;
    ...
    let release_id = release_id.or_else(|| {
        album
            .as_deref()
            .and_then(|album| search_release_for_album(album, artist.as_deref()).ok().flatten())
            .and_then(|release| release.release_id)
    });
    if let Some(release_id) = release_id {
        candidates.push(NativeAlbumArtworkCandidate {
            source: "web".to_string(),
            label: "Cover Art Archive".to_string(),
            path: None,
            track_id: None,
            artwork_url: Some(format!("{COVER_ART_ARCHIVE_ROOT}/release/{release_id}/front")),
            thumbnail_url: Some(format!("{COVER_ART_ARCHIVE_ROOT}/release/{release_id}/front-250")),
...
pub fn native_artwork_collision_repair(body: JsonValue) -> Result<JsonValue, String> {
    Ok(json!({
...
fn autotag_tracks(
    ...
    if !track_ids.is_empty() {
        ...
        let mut params = track_ids.into_iter().map(rusqlite::types::Value::from).collect::<Vec<_>>();
...
fn lookup_musicbrainz_track(track: &NativeTrack) -> Result<Option<TagProposal>, String> {
    let title = track
        .title
        .as_deref()
        .map(str::trim)
    ...
    if let Some(artist) = track.artist.as_deref().map(str::trim).filter(|value| !value.is_empty()) {
...
fn lookup_acoustid_track(track: &NativeTrack, api_key: &str) -> Result<Option<TagProposal>, String> {
...
fn save_album_completion(
    ...
    Ok(NativeAlbumCompletionLookupResponse {
        album_id,
        expected_track_count,
...
fn apply_tag_proposal(
    ...
    let _ = super::native_update_track_metadata(state, track_id, update, Some(false))?;
    Ok(())
}

fn changed_fields(current: &JsonValue, proposed: &Map<String, JsonValue>, missing_only: bool) -> Vec<String> {
    ...
    proposed
        ...
        .filter_map(|(field, value)| {
            ...
            if missing_only && old_text.as_deref().map(str::trim).filter(|v| !v.is_empty()).is_some() {
                return None;
            }
            (new_text != old_text && new_text.as_deref().map(str::trim).filter(|v| !v.is_empty()).is_some())
                .then(|| field.clone())
        })
        .collect()
}

fn track_current_json(track: &NativeTrack) -> JsonValue {
    json!({
        "title": track.title,
        "artist": track.artist,
        "album": track.album,
