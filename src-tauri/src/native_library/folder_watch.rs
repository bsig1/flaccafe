...
pub(crate) fn native_folder_watch_status(limit: usize) -> Result<FolderWatchStatus, String> {
    Ok(state()
        .lock()
...
pub(crate) fn native_start_folder_watch(body: JsonValue) -> Result<FolderWatchStatus, String> {
    ...
    native_refresh_folder_watch_with_files(Some(folder), limit, parse_native_files(&body))
}

pub(crate) fn native_stop_folder_watch(limit: usize) -> Result<FolderWatchStatus, String> {
    stop_watch_thread();
    {
        let connection = open_database()?;
...
pub(crate) fn native_refresh_folder_watch(body: JsonValue) -> Result<FolderWatchStatus, String> {
    let limit = body
        .get("limit")
        .and_then(JsonValue::as_u64)
        .map(|value| value.clamp(1, 5_000) as usize)
        .unwrap_or(DEFAULT_LIMIT);
    native_refresh_folder_watch_with_files(
        request_folder_path(&body)?,
        limit,
        parse_native_files(&body),
    )
}

pub(crate) fn native_apply_folder_watch(
    body: JsonValue,
) -> Result<FolderWatchApplyResponse, String> {
    let limit = body
        .get("limit")
        .and_then(JsonValue::as_u64)
        .map(|value| value.clamp(1, 5_000) as usize)
    ...
    if pending.is_empty() {
        return Ok(FolderWatchApplyResponse {
            ...
            skipped: 0,
            errors: Vec::new(),
            status: native_folder_watch_status(limit)?,
        });
    }
    let connection = open_database()?;
    ...
    let status = native_refresh_folder_watch_with_files(Some(folder), limit, None)?;
...
pub(crate) fn native_ack_folder_watch_notifications(
    body: JsonValue,
...
fn native_refresh_folder_watch_with_files(
    folder_path: Option<String>,
    limit: usize,
    ...
    match detect_folder_changes(&folder, native_files) {
        Ok(changes) => {
            let mut state = state()
                .lock()
                .map_err(|_| "Folder watch state is unavailable".to_string())?;
...
fn detect_folder_changes(
    ...
    let file_states = file_states_for_folder(&folder, native_files)?;
    let connection = open_database()?;
...
fn spawn_watch_thread(generation: u64, folder: String, interval_seconds: u64) {
    thread::spawn(move || loop {
        ...
        let _ = native_refresh_folder_watch_with_files(Some(folder.clone()), DEFAULT_LIMIT, None);
    });
}
...
fn parse_native_files(body: &JsonValue) -> Option<Vec<AudioSnapshot>> {
    body.get("native_snapshot")
        .and_then(JsonValue::as_object)
