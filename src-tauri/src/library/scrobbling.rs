...
fn account_from_row(row: &rusqlite::Row<'_>) -> rusqlite::Result<DesktopScrobbleAccount> {
    Ok(DesktopScrobbleAccount {
...
fn outbox_from_row(row: &rusqlite::Row<'_>) -> rusqlite::Result<DesktopScrobbleOutboxEntry> {
    Ok(DesktopScrobbleOutboxEntry {
        id: row.get("id")?,
        service: row.get::<_, Option<String>>("service")?.unwrap_or_default(),
...
fn fallback_account(service: &str) -> DesktopScrobbleAccount {
    DesktopScrobbleAccount {
...
fn account_by_service(
    ...
    service: &str,
) -> Result<DesktopScrobbleAccount, String> {
    connection
        .query_row(
...
#[tauri::command]
pub fn scrobble_accounts(
    _state: State<'_, DesktopLibraryState>,
) -> Result<Vec<DesktopScrobbleAccount>, String> {
...
#[tauri::command]
pub fn scrobble_outbox(
    _state: State<'_, DesktopLibraryState>,
    limit: Option<usize>,
) -> Result<Vec<DesktopScrobbleOutboxEntry>, String> {
    let connection = open_database()?;
...
#[tauri::command]
pub fn queue_scrobble_history(
    _state: State<'_, DesktopLibraryState>,
    service: String,
    limit: Option<usize>,
) -> Result<DesktopScrobbleQueueHistoryResponse, String> {
    let service = service.trim().to_ascii_lowercase();
    if !SERVICES.contains(&service.as_str()) {
        return Err("Scrobble service not found".to_string());
    }
    ...
    Ok(DesktopScrobbleQueueHistoryResponse {
        queued,
...
#[tauri::command]
pub fn submit_scrobble_outbox(
    _state: State<'_, DesktopLibraryState>,
    service: String,
    limit: Option<usize>,
) -> Result<DesktopScrobbleSubmitResponse, String> {
    let service = service.trim().to_ascii_lowercase();
    if !SERVICES.contains(&service.as_str()) {
        return Err("Scrobble service not found".to_string());
    }
    ...
    Ok(DesktopScrobbleSubmitResponse {
        submitted,
        failed,
...
#[tauri::command]
pub fn start_lastfm_login(
    _state: State<'_, DesktopLibraryState>,
    api_key: Option<String>,
    api_secret: Option<String>,
) -> Result<DesktopLastFmLoginStartResponse, String> {
    ...
    params.insert(
        "api_sig".to_string(),
        lastfm_signature(&params, &api_secret),
    );
    ...
    let token = payload
        ...
        .to_string();
    Ok(DesktopLastFmLoginStartResponse {
        auth_url: format!(
...
#[tauri::command]
pub fn complete_lastfm_login(
    _state: State<'_, DesktopLibraryState>,
    api_key: Option<String>,
    api_secret: Option<String>,
    token: String,
    enabled: Option<bool>,
) -> Result<DesktopLastFmLoginCompleteResponse, String> {
    let (api_key, api_secret) = resolve_lastfm_credentials(api_key, api_secret)?;
    ...
    Ok(DesktopLastFmLoginCompleteResponse { account })
}

#[tauri::command]
pub fn import_scrobbling_history(
    _state: State<'_, DesktopLibraryState>,
    path: String,
    apply: Option<bool>,
    limit: Option<usize>,
) -> Result<DesktopScrobbleHistoryImportResponse, String> {
    ...
    for (index, row) in reader.records().enumerate() {
        ...
        if artist.is_none() || title.is_none() {
            previews.push(DesktopScrobbleHistoryImportPreview {
                row: row_number,
                matched: false,
                track_id: None,
                artist,
    ...
    Ok(DesktopScrobbleHistoryImportResponse {
...
fn listenbrainz_payload(rows: &[DesktopScrobbleOutboxEntry]) -> serde_json::Value {
    let payload = rows
        .iter()
        .map(|row| {
            json!({
...
fn submit_listenbrainz(
    rows: &[DesktopScrobbleOutboxEntry],
    account: &DesktopScrobbleAccount,
) -> Result<(), String> {
    let token = account
        .token
        .as_deref()
...
fn submit_lastfm(
    row: &DesktopScrobbleOutboxEntry,
    account: &DesktopScrobbleAccount,
) -> Result<(), String> {
