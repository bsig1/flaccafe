...
pub fn import_external_library_stats(
    source: String,
    import_path: String,
    apply: Option<bool>,
    missing_only: Option<bool>,
    limit: Option<usize>,
) -> Result<DesktopLibraryStatsImportResponse, String> {
    ...
    for record in &records {
            ...
            if !changed_fields.is_empty() {
                ...
                if apply.unwrap_or(false) {
                    connection
                        ...
                        .map_err(|error| {
                            format!("Could not apply imported library stats: {error}")
                        })?;
        ...
        previews.push(DesktopLibraryStatsImportPreview {
            ...
            current_last_played_at: track
                .as_ref()
                .and_then(|track| track.last_played_at.clone()),
            changed_fields,
            error,
        });
    }
    Ok(DesktopLibraryStatsImportResponse {
        source,
        import_path,
...
fn parse_musicbee_csv(path: &Path, limit: usize) -> Result<Vec<ImportedStat>, String> {
    ...
    for (index, row) in reader.records().enumerate() {
        ...
        records.push(ImportedStat {
            ...
            play_count: csv_value(
                &headers,
                &row,
                &["play count", "plays", "played", "playcount"],
            )
            .and_then(|value| parse_int(&value)),
            last_played_at: csv_value(
                &headers,
                &row,
                &["last played", "last played at", "lastplayed"],
            ),
            error: None,
        });
    }
...
fn parse_itunes_xml(path: &Path, limit: usize) -> Result<Vec<ImportedStat>, String> {
    ...
    for (index, track) in tracks.values().enumerate() {
        ...
        records.push(ImportedStat {
            ...
            artist: plist_string(dict.get("Artist"))
                .or_else(|| plist_string(dict.get("Album Artist"))),
...
fn parse_windows_media_player_xml(path: &Path, limit: usize) -> Result<Vec<ImportedStat>, String> {
    let payload = std::fs::read_to_string(path)
        .map_err(|error| format!("Could not read WMP XML: {error}"))?;
    let document = roxmltree::Document::parse(&payload)
        .map_err(|error| format!("Invalid WMP XML: {error}"))?;
    let mut records = Vec::new();
        ...
        .enumerate()
    {
        ...
        records.push(ImportedStat {
            ...
            rating: attr_any(node, &["userrating", "rating"])
                .and_then(|value| parse_rating(&value)),
