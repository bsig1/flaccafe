...
fn inbox_note_from_row(row: &rusqlite::Row<'_>) -> rusqlite::Result<DesktopInboxTrackNote> {
    Ok(DesktopInboxTrackNote {
...
fn inbox_rule_from_row(row: &rusqlite::Row<'_>) -> rusqlite::Result<DesktopInboxAutoReviewRule> {
    Ok(DesktopInboxAutoReviewRule {
        id: row.get("id")?,
...
fn list_inbox_notes(
    ...
    track_ids: &[i64],
) -> Result<Vec<DesktopInboxTrackNote>, String> {
...
fn list_auto_review_rules(
    connection: &Connection,
) -> Result<Vec<DesktopInboxAutoReviewRule>, String> {
    let mut statement = connection
        .prepare(
            r#"
...
fn auto_review_rule_by_id(
    connection: &Connection,
    rule_id: i64,
) -> Result<DesktopInboxAutoReviewRule, String> {
    connection
        .query_row(
            r#"
            SELECT id, name, enabled, field, match_type, value, note, created_at, updated_at
...
fn rule_matches(rule: &DesktopInboxAutoReviewRule, value: &str) -> bool {
...
pub fn inbox(
    ...
    limit: Option<usize>,
    offset: Option<usize>,
) -> Result<DesktopInboxResponse, String> {
...
#[tauri::command]
pub fn update_inbox_note(
    _state: State<'_, DesktopLibraryState>,
    track_id: i64,
    note: Option<String>,
) -> Result<Option<DesktopInboxTrackNote>, String> {
...
pub fn review_inbox(
    ...
    all_new: Option<bool>,
) -> Result<DesktopInboxReviewResponse, String> {
    let mut connection = open_database()?;
    let transaction = connection
        .transaction()
...
#[tauri::command]
pub fn inbox_auto_review_rules(
    _state: State<'_, DesktopLibraryState>,
) -> Result<Vec<DesktopInboxAutoReviewRule>, String> {
    let connection = open_database()?;
...
pub fn create_inbox_auto_review_rule(
    _state: State<'_, DesktopLibraryState>,
    name: String,
    enabled: Option<bool>,
    field: String,
...
) -> Result<DesktopInboxAutoReviewRuleApplyResponse, String> {
    let mut connection = open_database()?;
    let transaction = connection
        .transaction()
    ...
    let (total_new, total_reviewed) = inbox_counts(&connection)?;
    Ok(DesktopInboxAutoReviewRuleApplyResponse {
        rule,
        applied,
        total_new,
        total_reviewed,
...
#[tauri::command]
#[allow(clippy::too_many_arguments)]
pub fn update_inbox_auto_review_rule(
    _state: State<'_, DesktopLibraryState>,
    rule_id: i64,
    name: String,
    enabled: Option<bool>,
...
#[tauri::command]
pub fn delete_inbox_auto_review_rule(
    _state: State<'_, DesktopLibraryState>,
    rule_id: i64,
) -> Result<DesktopInboxAutoReviewRuleDeleteResponse, String> {
    let connection = open_database()?;
    let deleted = connection
        ...
        > 0;
    let (total_new, total_reviewed) = inbox_counts(&connection)?;
    Ok(DesktopInboxAutoReviewRuleDeleteResponse {
        deleted,
        total_new,
        total_reviewed,
    })
