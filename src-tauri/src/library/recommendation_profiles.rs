use super::recommendations::{autodj_settings, generate_autodj_response};
use super::types::*;
...
fn recommendation_drift_from_value(value: serde_json::Value) -> DesktopRecommendationDrift {
    let warnings = value
        .get("warnings")
...
fn recommendation_profile_from_row(
    row: &rusqlite::Row<'_>,
) -> rusqlite::Result<DesktopRecommendationProfile> {
    let settings_json = value_from_text(row.get("settings_json")?, json!({}));
    Ok(DesktopRecommendationProfile {
        id: row.get("id")?,
        name: row.get::<_, Option<String>>("name")?.unwrap_or_default(),
        settings: autodj_settings(settings_json),
...
fn raw_profile_from_row(
    row: &rusqlite::Row<'_>,
) -> rusqlite::Result<(DesktopRecommendationProfile, serde_json::Value)> {
    let settings_json = value_from_text(row.get("settings_json")?, json!({}));
    Ok((
        DesktopRecommendationProfile {
            id: row.get("id")?,
            name: row.get::<_, Option<String>>("name")?.unwrap_or_default(),
            settings: autodj_settings(settings_json.clone()),
            is_default: row.get::<_, Option<i64>>("is_default")?.unwrap_or(0) != 0,
            created_at: row
                .get::<_, Option<String>>("created_at")?
                .unwrap_or_default(),
...
fn recommendation_run_from_row(
    row: &rusqlite::Row<'_>,
) -> rusqlite::Result<DesktopRecommendationRun> {
    let settings_json = value_from_text(row.get("settings_json")?, json!({}));
    ...
    let track_ids = row
        ...
        .unwrap_or_default();
    Ok(DesktopRecommendationRun {
        id: row.get("id")?,
        settings: autodj_settings(settings_json),
        drift: recommendation_drift_from_value(drift_json),
        track_ids,
        created_at: row
            .get::<_, Option<String>>("created_at")?
            .unwrap_or_default(),
...
) -> Result<Vec<DesktopRecommendationProfile>, String> {
...
fn profile_by_id(
    connection: &Connection,
    profile_id: i64,
) -> Result<DesktopRecommendationProfile, String> {
    connection
        .query_row(
            r#"
            SELECT id, name, settings_json, is_default, created_at, updated_at
...
#[tauri::command]
pub fn recommendation_profiles(
...
pub fn recommendation_history(
    _state: State<'_, DesktopLibraryState>,
    limit: Option<usize>,
) -> Result<Vec<DesktopRecommendationRun>, String> {
    let connection = open_database()?;
...
#[tauri::command]
pub fn save_recommendation_profile(
    _state: State<'_, DesktopLibraryState>,
...
#[tauri::command]
pub fn set_default_recommendation_profile(
    _state: State<'_, DesktopLibraryState>,
    profile_id: i64,
) -> Result<Vec<DesktopRecommendationProfile>, String> {
    let mut connection = open_database()?;
...
fn challenger_settings(base: &serde_json::Value) -> serde_json::Value {
    let base_settings = autodj_settings(base.clone());
...
pub fn create_recommendation_ab_test(
    ...
    Ok(DesktopRecommendationAbTestResponse {
        test_id: test_id(),
        generated_at: now_label(),
        queues: vec![
            DesktopRecommendationAbQueue {
                label: "A".to_string(),
...
#[tauri::command]
pub fn choose_recommendation_ab_test(
    _state: State<'_, DesktopLibraryState>,
    chosen_label: String,
    chosen_track_ids: Vec<i64>,
    feedback_weight: Option<f64>,
) -> Result<DesktopRecommendationAbChoiceResponse, String> {
    ...
    if unique_ids.is_empty() {
        return Ok(DesktopRecommendationAbChoiceResponse {
            status: "ok".to_string(),
            chosen_label,
            inserted_feedback: 0,
        });
    ...
    Ok(DesktopRecommendationAbChoiceResponse {
        status: "ok".to_string(),
        chosen_label,
...
) -> Result<Vec<(DesktopRecommendationProfile, serde_json::Value)>, String> {
...
fn build_profile_comparisons(
    ...
    seed_track_id: Option<i64>,
    seed: Option<i64>,
) -> Result<Vec<DesktopRecommendationProfileComparison>, String> {
    let connection = open_database()?;
    ...
    for (profile, mut settings_json) in rows.into_iter().take(8) {
        ...
        comparisons.push(DesktopRecommendationProfileComparison {
            profile,
...
#[tauri::command]
pub fn compare_recommendation_profiles(
    _state: State<'_, DesktopLibraryState>,
    profile_ids: Option<Vec<i64>>,
    seed_track_id: Option<i64>,
    seed: Option<i64>,
...
#[tauri::command]
pub fn export_recommendation_profile_comparison(
    _state: State<'_, DesktopLibraryState>,
    profile_ids: Option<Vec<i64>>,
    seed_track_id: Option<i64>,
    seed: Option<i64>,
) -> Result<DesktopRecommendationProfileComparisonExportResponse, String> {
    ...
    Ok(DesktopRecommendationProfileComparisonExportResponse {
...
#[tauri::command]
pub fn import_recommendation_profile_comparison(
    _state: State<'_, DesktopLibraryState>,
    report_path: String,
) -> Result<DesktopRecommendationProfileComparisonImportResponse, String> {
    ...
    for item in raw {
        comparisons.push(
            serde_json::from_value::<DesktopRecommendationProfileComparison>(item.clone())
                .map_err(|error| format!("Invalid comparison entry: {error}"))?,
        );
    }
    Ok(DesktopRecommendationProfileComparisonImportResponse {
