...
pub(super) const TRACK_COLUMNS: &str = "
    ...
    analysis_mood, analysis_mood_confidence, analysis_mood_tags,
...
pub(super) fn track_from_row(row: &rusqlite::Row<'_>) -> rusqlite::Result<DesktopTrack> {
    Ok(DesktopTrack {
        ...
        analysis_mood: row.get("analysis_mood")?,
        analysis_mood_confidence: row.get("analysis_mood_confidence")?,
        analysis_mood_tags: row.get("analysis_mood_tags")?,
