use super::{
    fade_frame_count, smooth_fade_progress, DesktopGainControl, DesktopVisualizerState,
...
{
    player.append(DesktopDspSource::new(
        source,
        dsp_settings,
        gain,
        visualizer,
...
#[derive(Clone, Copy)]
pub(super) enum DesktopEqBandKind {
    LowShelf,
    Peaking,
    HighShelf,
}
...
pub(super) struct DesktopDspSource<S>
where
    S: Source<Item = f32>,
{
    input: S,
    settings: Arc<Mutex<DesktopDspSettings>>,
    gain_control: DesktopGainControl,
    visualizer: Arc<Mutex<DesktopVisualizerState>>,
    ...
    S: Source<Item = f32>,
{
    ...
    pub(super) fn new(
        ...
        let active_settings = settings
            ...
            .unwrap_or_else(|_| DesktopDspSettings::default().normalized());
        let mut source = Self {
            input,
            settings,
            gain_control,
...
impl<S> Drop for DesktopDspSource<S>
where
    S: Source<Item = f32>,
{
    fn drop(&mut self) {
...
impl<S> Iterator for DesktopDspSource<S>
where
    S: Source<Item = f32>,
{
    type Item = f32;
...
impl<S> Source for DesktopDspSource<S>
where
    S: Source<Item = f32>,
{
