...
- Rust implementation: add the native route handler in `src-tauri/src/python_worker/native_routes.rs` or a focused `src-tauri/src/native_library/` module.
...
- Fallback removal: remove the Python action only after the native path has parity coverage and no known feature loss.
