...
const COLLECTION_TRACK_CACHE_LIMIT = 12;
...
export function useAppController() {
  ...
  function rememberCollectionTracks<K>(cache: Map<K, Track[]>, key: K, collectionTracks: Track[]) {
    cache.delete(key);
    cache.set(key, collectionTracks);
    while (cache.size > COLLECTION_TRACK_CACHE_LIMIT) {
  ...
  function updateCollectionTrackCaches(updater: (track: Track) => Track | null) {
    for (const [albumId, cachedTracks] of selectedAlbumTracksCacheRef.current.entries()) {
      const updatedTracks = cachedTracks.map(updater).filter((track): track is Track => Boolean(track));
      if (updatedTracks.length > 0) {
        selectedAlbumTracksCacheRef.current.set(albumId, updatedTracks);
      } else {
        selectedAlbumTracksCacheRef.current.delete(albumId);
      }
    }
    for (const [artistName, cachedTracks] of selectedArtistTracksCacheRef.current.entries()) {
      const updatedTracks = cachedTracks.map(updater).filter((track): track is Track => Boolean(track));
      if (updatedTracks.length > 0) {
        selectedArtistTracksCacheRef.current.set(artistName, updatedTracks);
      } else {
        selectedArtistTracksCacheRef.current.delete(artistName);
      }
    }
  }
  ...
  function replaceTrackEverywhere(updated: Track, options?: { lightweightLibraryCache?: boolean }) {
    ...
    updateCollectionTrackCaches(replace);
  ...
  function removeTrackEverywhere(trackId: number) {
    ...
    updateCollectionTrackCaches((track) => (remove(track) ? track : null));
  ...
  const currentFileTagWriteHoldTrackId =
    currentTrack && currentTrack.id > 0 && !currentTrack.is_preview ? currentTrack.id : null;
  const { queueFileTagWrite } = useDeferredFileTagWriter({
    currentTrackId: currentFileTagWriteHoldTrackId,
    setStatus,
  });
  ...
  const playerNavigationHandlers = createPlayerNavigationHandlers({ albums, artistInfo, currentTrack, fetchAlbums, fetchArtistInfo, fetchArtistLocalTracks, findAlbumForTrack, handleSelectAlbum, handleSelectArtist, primaryArtistName, splitArtistNames, saveArtistInfoOverride, setActivePage, setAlbums, setArtistInfo, setArtistTracks, setDebouncedSearch, setDetailTrack, setIsArtistLoading, setLibraryView, setSearch, setStatus, uiPreferences });
  const { handleOpenCurrentTrackFromPlayer, handleOpenCurrentArtistFromPlayer, handleOpenCurrentArtistInfoFromPlayer, handleOpenCurrentAlbumFromPlayer, loadArtistInfo, handleSaveArtistInfoOverride } = playerNavigationHandlers;
