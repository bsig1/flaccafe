...
export function LibraryHealthBranch({ model }: { model: any }) {
  const { ArrowDown, ArrowUp, CheckCircle2, Pencil, Play, Trash2, Wand2, RatingStars, display, fileName, formatBitrate, formatDuration, formatFingerprint, formatTime, missingMetadataFields, missingMetadataFilters, libraryStats, libraryHealth, libraryView, onRating, onPlayTrack, onDeleteTrack, onEditTrack, onRequestDeleteTracks, setDetailTrack, onAnalyzeTracks, onIgnoreDuplicateGroup, onClearIgnoredDuplicateGroups, isAudioAnalyzing, displayRatingsAsNumbers, showAllDuplicateGroups, setShowAllDuplicateGroups, showAllMissingMetadata, setShowAllMissingMetadata, missingMetadataFilter, setMissingMetadataFilter, missingMetadataRows, filteredMissingMetadataRows, visibleMissingMetadataRows, visibleDuplicateGroups } = model;
                        ...
                        <div className="rounded border border-line">
                          {unratedTracks.slice(0, 12).map((track: any) => (
                            <div key={track.id} className="grid gap-2 border-b border-line/60 px-3 py-2 last:border-b-0 min-[760px]:grid-cols-[minmax(0,1fr)_auto] min-[760px]:items-center">
                              ...
                              <RatingStars rating={track.rating} displayAsNumber={displayRatingsAsNumbers} onChange={(rating: any) => onRating(track.id, rating)} />
                            </div>
                          ))}
