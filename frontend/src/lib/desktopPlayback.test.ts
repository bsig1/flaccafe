...
describe("Rust playback diagnostics", () => {
  it("summarizes empty diagnostics", () => {
    expect(summarizePlaybackDiagnostics(null)).toBe("Diagnostics not loaded");
    expect(summarizePlaybackDiagnostics(diagnostics([]))).toBe("No recent Rust playback failures");
  });

  it("summarizes recent failures by severity and category", () => {
    const summary = summarizePlaybackDiagnostics(
      diagnostics(
        [
          {
            id: 1,
